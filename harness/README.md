...
A small set of shell scripts that bring up Alpha + deciduous side by side from one checkout. Each recipe is exposed in the repo-root `justfile` (`just harness-doctor`, `just harness-up`, `just harness-status`, etc.).
...
│                                                                    │ │
│  reads .claude/agents/Alpha.md          ◄── persona                │ │
...
| `harness-up`        | Idempotent startup. Bring up dev stack → optionally start alpha-server (if `.env` is complete) → render system prompts (if Alpha-System-Prompts is present) → status. |
| `harness-down`      | Graceful teardown. Stops alpha-server (PID file), then dev stack. Data volumes preserved.                                                                          |
| `harness-sync`      | Pull the latest Alpha-owned `.claude/` payload (agents, context, rules, start skill, `.mcp.json`) from the sibling `../Alpha-dotclaude` checkout into here. Preserves deciduous's `commands/`, `hooks/`, `skills/`. Reports if `settings.json`'s merge is missing any of the five expected sections. |
| `server-up`         | Start just alpha-server in the background (assumes dev stack already up). PID in `harness/alpha-server.pid`.                                                       |
| `prompt-render`     | Run `Alpha-System-Prompts/render.py` to produce `docs/system-prompts/claude_code.md` and `claude_desktop.md`.                                                       |

## Sibling-repo expectations

The harness assumes both companion repos are cloned beside `Alpha/`:
...
- **No `Alpha-System-Prompts/`**: prompt rendering is skipped. The live persona doctrine is in `.claude/agents/Alpha.md` regardless — the Alpha-System-Prompts repo is the legacy layer.
- **No `Alpha-dotclaude/`**: `harness-sync` is unavailable. The integrated `.claude/` already in this checkout still works; you just can't pull updates.
...
```
.claude/
├── agents/      Alpha.md, Answertron.md, Librarian.md        ◄ from Alpha-dotclaude
├── context/     identity.md, household.md, lexicon.md        ◄ from Alpha-dotclaude
├── rules/       python.md, workshop.md (path-scoped)         ◄ from Alpha-dotclaude
├── skills/start/SKILL.md                                      ◄ from Alpha-dotclaude
...
| ------------------ | ---------------------------------------------- | ----------- |
| `UserPromptSubmit` | `http://localhost:8000/hooks/timestamp`        | Alpha       |
| `UserPromptSubmit` | `http://localhost:8000/hooks/memories`         | Alpha       |
...
`Alpha-System-Prompts/render.py` reads the input templates and emits `claude_code.md` / `claude_desktop.md` into `docs/system-prompts/`. These are reference artifacts (legacy persona layer); the live persona is in `.claude/agents/Alpha.md`.

## Why this is a harness and not a server

The name follows Alpha's own usage. From `agents/Alpha.md`:
